...
import { Mic, Play, Download, RotateCcw, Loader2, Wand2, Volume2, Star, Clapperboard } from "lucide-react";
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="rounded-xl border border-border bg-card p-5 flex flex-col gap-5">
              ...
              <div className="space-y-2.5">
                ...
                {(() => {
                  const active = NARRATOR_PRESETS.find((p) => p.speed === speed[0] && p.pitch === pitch[0]);
                  return active ? (
                    <p className="text-[10px] text-muted-foreground italic">{active.description}</p>
                  ) : null;
                })()}
              </div>
